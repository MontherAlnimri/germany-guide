  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <button onClick={() => setShowForm(!showForm)} className="px-4 py-2 bg-blue-600 text-white text-sm rounded-xl font-medium hover:bg-blue-700 transition-colors">
        ...
        <div className="bg-white rounded-xl border border-gray-100 p-6">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              ...
              <input type="text" value={title} onChange={(e) => setTitle(e.target.value)} placeholder={dict.deadlines.titlePlaceholder || "e.g. Visa Renewal"} className="w-full px-4 py-2.5 border border-gray-200 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-transparent outline-none" />
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">{dict.deadlines.description}</label>
              <input type="text" value={description} onChange={(e) => setDescription(e.target.value)} placeholder={dict.deadlines.descPlaceholder || "Add details about this deadline..."} className="w-full px-4 py-2.5 border border-gray-200 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-transparent outline-none" />
            </div>
      ...
      <div>
        <h2 className="text-lg font-semibold text-gray-900 mb-3">{dict.deadlines.pending || "Pending"}</h2>
        {pending.length === 0 ? (
