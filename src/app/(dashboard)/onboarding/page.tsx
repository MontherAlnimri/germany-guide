...
export default function OnboardingPage() {
  ...
  const handleSubmit = async () => {
    if (!fullName || !visaType || !applicationType) { setError(dict.onboarding.fillAllFields); return; }
    ...
    <div className="max-w-lg mx-auto py-8">
      <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-8">
        <h1 className="text-2xl font-bold text-gray-900 mb-2">
          {isUpdate ? dict.onboarding.updateProfile : dict.onboarding.welcome}
        </h1>
        <div className="flex items-center gap-2 mb-6">
          <span className="text-sm text-gray-500">{dict.onboarding.step} {step} {dict.onboarding.of} 3</span>
        ...
        {step === 1 && (
          <div className="space-y-4">
            ...
            <p className="text-sm text-gray-600">{dict.onboarding.aboutYouDesc}</p>
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">{dict.onboarding.fullName}</label>
              <input type="text" value={fullName} onChange={(e) => setFullName(e.target.value)} placeholder={dict.onboarding.fullNamePlaceholder} className="w-full px-4 py-2.5 border border-gray-200 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-transparent outline-none" />
            </div>
            <div>
              ...
              <select value={visaType} onChange={(e) => setVisaType(e.target.value)} className="w-full px-4 py-2.5 border border-gray-200 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-transparent outline-none bg-white">
                <option value="">{dict.onboarding.visaTypePlaceholder}</option>
            ...
            <div>
              ...
              <select value={applicationType} onChange={(e) => setApplicationType(e.target.value)} className="w-full px-4 py-2.5 border border-gray-200 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-transparent outline-none bg-white">
                <option value="">{dict.onboarding.applicationTypePlaceholder}</option>
                {APPLICATION_TYPES.map((a) => <option key={a} value={a}>{getVisaLabel(a)}</option>)}
              </select>
            </div>
        ...
        {step === 2 && (
          <div className="space-y-4">
            ...
            <p className="text-sm text-gray-600">{dict.onboarding.locationDesc}</p>
            <div>
              ...
              <select value={city} onChange={(e) => setCity(e.target.value)} className="w-full px-4 py-2.5 border border-gray-200 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-transparent outline-none bg-white">
                <option value="">{dict.onboarding.cityPlaceholder}</option>
            ...
            <div>
              ...
              <input type="text" value={zipCode} onChange={(e) => setZipCode(e.target.value)} placeholder={dict.onboarding.zipCodePlaceholder} maxLength={5} className="w-full px-4 py-2.5 border border-gray-200 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-transparent outline-none" />
              <p className="text-xs text-gray-500 mt-1">{dict.onboarding.zipCodeHint}</p>
            </div>
        ...
        {step === 3 && (
          <div className="space-y-4">
            <h2 className="text-lg font-semibold text-gray-900">{dict.onboarding.visaExpiry}</h2>
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">{dict.onboarding.visaExpiryLabel}</label>
              ...
              <p className="text-xs text-gray-500 mt-1">{dict.onboarding.visaExpiryHint}</p>
            </div>
            <div className="flex gap-3">
